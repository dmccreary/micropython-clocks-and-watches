...
# clock = FourDigitClock(ser_pin=SERIAL_DATA_PIN, srclk_pin=SRCLK_PIN, rclk_pin=RCLK_PIN)
RCLK_PIN = 0
SRCLK_PIN = 1
SERIAL_DATA_PIN = 2
def wait_for_localtime():
    
    print("Waiting for a localtime to work")
    sleep(.25)
    # rtc = RTC()
    while True:
        ...
        if current_time[0] != 2021 or current_time[1] != 1 or current_time[2] != 1:
            print("Localtime is ready!")
            return
        print("Localtime is not ready yet...")
